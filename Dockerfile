...
RUN apt-get update && \
    ...
    useradd rust --uid 1001 --user-group --create-home --shell /bin/bash --groups sudo
...
cd kernel-headers && sudo make ARCH=x86_64 prefix=/usr/local/musl install

RUN cd /home/rust/libs && \
curl -LO http://www.tcpdump.org/release/libpcap-1.7.4.tar.gz && \
tar -zxvf libpcap-1.7.4.tar.gz && cd libpcap-1.7.4 && \
CC=musl-gcc ./configure --prefix=/usr/local/musl && \
make && sudo make install-archive && \
cd .. && rm -rf libpcap-*
